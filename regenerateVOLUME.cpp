// Run this code to completely regenerate the lines.txt, spectra.txt, and repoire.json files
// Necessary if things got totally messed up somehow
...
int main(int argc, char* argv[])
{
   ...
   //set up to make "spectrum.txt"
   fstream spectrum_file;
   spectrum_file.open (DATADIR_SPECTRUM, fstream::out | fstream::trunc);

   //set up to make "lines.txt" 
   fstream line_file;
   line_file.open(DATADIR_LINES, fstream::out | fstream::trunc);
   ...
   for (map<File, set<int>, FileCompare >::iterator itx = keyLists.begin();   // iterate over files
        itx != keyLists.end(); ++itx)
   {
      ...
      { 
         ++ycount;
         if (xcount >= ycount) { continue;}  // only consider unique pairs
   ...
   line_file.close();
   spectrum_file.close();
   WriteRepoireJSON();    // Write the JSON file used for the force network
   return 0;
}
